...
class OAuthService {
  constructor(private client: ReturnType<typeof axios.create>) {
    if (ENV.oAuthServerUrl) {
      ...
      console.log(
        "[Auth] OAUTH_SERVER_URL is not set. Built-in institutional authentication is active."
      );
    }
  }
...
class SDKServer {
  ...
  private getSessionSecret() {
    const secret = ENV.cookieSecret || "dev_secret_estudo_bndes_jwt_key_default_32chars_long";
  ...
  async authenticateRequest(req: Request): Promise<AuthenticatedUser> {
    ...
    if (!session) {
      if (!ENV.isProduction && !ENV.oAuthServerUrl) {
        let user = await db.getUserByOpenId("local_admin");
        if (!user) {
          try {
            await db.upsertUser({
              openId: "local_admin",
            ...
            user = await db.getUserByOpenId("local_admin");
          } catch {
            // DB fallback
          }
        }
    ...
    try {
      ...
      // Ignored if DB is in local mock mode
    }
