...
function CalendarContent() {
  ...
  // Estrutura hierárquica oficial: Tomo -> Capítulo -> Seções / Atividades
  type ChapterGroup = {
    sectionCode: string;
    sectionTitle: string;
    tome: string;
    groupName: string | null;
    responsibleName: string;
    parentItem: (typeof filtered)[number] | null;
  ...
  const tomeGroups = useMemo(() => {
    // 1. Agrupar itens pelo código do capítulo/seção canônica
    ...
    filtered.forEach(item => {
      ...
      const title = def?.title || (item.parentActivityId === null ? item.title : `Capítulo ${code}`);
      ...
      if (!chapterMap.has(code)) {
        chapterMap.set(code, {
          ...
          parentItem: item.parentActivityId === null ? item : null,
          rawItems: [],
          sortOrder,
        });
      }
      const entry = chapterMap.get(code)!;
      if (item.parentActivityId === null && !entry.parentItem) {
        entry.parentItem = item;
        if (item.responsibleName) entry.responsibleName = item.responsibleName;
        if (item.groupName) entry.groupName = item.groupName;
      }
      entry.rawItems.push(item);
    });

    // 2. Construir capítulos: se houver seções filhas, NÃO repetir o título do capítulo como atividade
    const processedChapters: ChapterGroup[] = Array.from(chapterMap.values()).map(ch => {
      const childActivities = ch.rawItems.filter(item => item.parentActivityId !== null);
      // Título não é uma atividade: se houver seções filhas abaixo do capítulo, usar apenas elas
      const items = childActivities.length > 0 ? childActivities : ch.rawItems;
      const completedCount = items.filter(i => i.status === "concluído").length;
      const delayedCount = items.filter(i => i.status === "atrasado").length;
      const avgProgress = Math.round(
        items.reduce((acc, i) => acc + i.progress, 0) / Math.max(1, items.length)
      );
      ...
      return {
        ...
        tome: ch.tome,
        groupName: ch.groupName ?? ch.parentItem?.groupName ?? null,
        responsibleName: ch.responsibleName || ch.parentItem?.responsibleName || "Responsável não informado",
        parentItem: ch.parentItem,
        items,
    ...
    // 3. Agrupar capítulos por Tomo
    const tomeOrderMap: Record<string, number> = {
      "Apresentação": 0,
      "Tomo I": 1,
      "Tomo II": 2,
      "Tomo III": 3,
    ...
    const tomeMap = new Map<string, TomeGroup>();

    processedChapters.forEach(chapter => {
      const tomeKey = chapter.tome || "Outro";
      if (!tomeMap.has(tomeKey)) {
        const tomeTitle =
          STUDY_TOME_TITLES[tomeKey as StudyTome] ||
          (tomeKey === "Outro" ? "Outros Itens" : tomeKey);
        tomeMap.set(tomeKey, {
          tomeCode: tomeKey,
          tomeTitle,
          sortOrder: tomeOrderMap[tomeKey] ?? 99,
          chapters: [],
          totalActivities: 0,
          completedCount: 0,
          delayedCount: 0,
      ...
      tomeMap.get(tomeKey)!.chapters.push(chapter);
    });

    // 4. Calcular métricas agregadas do Tomo
    const tomos = Array.from(tomeMap.values()).map(t => {
      const allActivities = t.chapters.flatMap(c => c.items);
      const totalActivities = allActivities.length;
      const completedCount = allActivities.filter(i => i.status === "concluído").length;
      const delayedCount = allActivities.filter(i => i.status === "atrasado").length;
      const avgProgress = Math.round(
        allActivities.reduce((acc, i) => acc + i.progress, 0) / Math.max(1, totalActivities)
      );
    ...
    return tomos.sort((a, b) => a.sortOrder - b.sortOrder);
  }, [filtered]);

  const allChapters = useMemo(() => tomeGroups.flatMap(t => t.chapters), [tomeGroups]);
  ...
  // Controle de expansão de Tomos (abertos por padrão para visibilidade executiva)
  const [expandedTomos, setExpandedTomos] = useState<Record<string, boolean>>({});

  const isTomeExpanded = (tomeCode: string) => {
    return expandedTomos[tomeCode] !== false;
  };

  const toggleTome = (tomeCode: string) => {
    setExpandedTomos(prev => ({
      ...prev,
      [tomeCode]: prev[tomeCode] === false ? true : false,
    }));
  };
  ...
  const expandAll = () => {
    ...
    const updatedTomos: Record<string, boolean> = {};
    tomeGroups.forEach(t => {
      updatedTomos[t.tomeCode] = true;
      t.chapters.forEach(c => {
        updatedSections[c.sectionCode] = true;
      });
    });
    setExpandedTomos(updatedTomos);
    setExpandedSections(updatedSections);
    toast.success("Todos os tomos e capítulos foram expandidos.");
  };

  const collapseAll = () => {
    setExpandedSections({});
    toast.success("Todos os capítulos foram compactados.");
  };
    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 px-1 text-xs">
        <div className="flex flex-wrap items-center gap-1.5">
          <Badge variant="outline" className="rounded-sm font-mono text-[10px] h-5.5">
            {tomeGroups.length} {tomeGroups.length === 1 ? "tomo" : "tomos"}
          </Badge>
          <Badge variant="outline" className="rounded-sm font-mono text-[10px] h-5.5">
      ...
      <div ref={timelineRef} className="bg-[#f8f8f5] dark:bg-card/40 rounded-lg">
        <section className="hidden overflow-hidden rounded-md border bg-card lg:block shadow-xs">
          ...
          {/* Lista de Tomos, Capítulos e Seções com Acordeão */}
          <div className="max-h-[75vh] overflow-y-auto overflow-x-auto divide-y divide-border/60">
            {tomeGroups.map(tome => {
              const isTomeOpen = isTomeExpanded(tome.tomeCode);

              return (
                <div key={tome.tomeCode} className="border-b border-border/80 last:border-b-0">
                  {/* Nível 1: Cabeçalho do Tomo (Nível Superior) */}
                  <div
                    onClick={() => toggleTome(tome.tomeCode)}
                    className="flex items-center justify-between gap-3 bg-muted/85 hover:bg-muted px-4 py-2.5 cursor-pointer transition-colors select-none sticky top-0 z-30 backdrop-blur-md border-y border-border/80"
                  >
                    <div className="flex items-center gap-2.5 min-w-0">
                      <Button
                        ...
                        aria-label={isTomeOpen ? `Recolher ${tome.tomeCode}` : `Expandir ${tome.tomeCode}`}
                      >
                        {isTomeOpen ? (
                          <ChevronDown className="h-4 w-4 transition-transform text-primary font-bold" />
                        ) : (
                          <ChevronRight className="h-4 w-4 transition-transform font-bold" />
                        )}
                      </Button>

                      <div className="flex flex-wrap items-center gap-2 min-w-0">
                        <span className="font-mono text-xs font-bold px-2 py-0.5 rounded bg-primary text-primary-foreground shrink-0 shadow-xs">
                          {tome.tomeCode}
                        </span>
                        <h2 className="font-bold text-xs text-foreground tracking-tight truncate">
                          {tome.tomeTitle}
                        </h2>
                      </div>
                    </div>
                  ...
                  {/* Nível 2: Capítulos dentro do Tomo */}
                  {isTomeOpen && (
                    <div className="divide-y divide-border/40">
                      {tome.chapters.map(chapter => {
                        const expanded = isSectionExpanded(chapter.sectionCode);

                        return (
                          <div key={chapter.sectionCode} className="divide-y divide-border/40">
                            {/* Cabeçalho do Capítulo (Nível Superior) */}
                            <div
                              onClick={() => toggleSection(chapter.sectionCode)}
                              className="flex items-center justify-between gap-3 bg-muted/40 hover:bg-muted/70 px-4 py-2 cursor-pointer transition-colors select-none sticky top-9 z-20 backdrop-blur-sm border-b border-border/50 pl-7 sm:pl-9"
                            >
                              <div className="flex items-center gap-2.5 min-w-0">
                                <Button
                                  type="button"
                                  ...
                                  size="icon"
                                  className="h-5 w-5 shrink-0 p-0 text-muted-foreground hover:text-foreground"
                                  aria-label={expanded ? `Recolher capítulo ${chapter.sectionCode}` : `Expandir capítulo ${chapter.sectionCode}`}
                                >
                                  {expanded ? (
                              ...
                              <div className="flex items-center gap-3 shrink-0">
                                <div className="hidden md:flex items-center gap-1.5 text-[11px] text-muted-foreground">
                                  ...
                                  <span>·</span>
                                  <span className="text-emerald-700 dark:text-emerald-400 font-medium">
                                    {chapter.completedCount} concluídas
                                  </span>
                                  {chapter.delayedCount > 0 && (
                                    <>
                                      <span>·</span>
                                      <span className="text-red-700 dark:text-red-400 font-medium">
                                        {chapter.delayedCount} atrasadas
                                      </span>
                                    </>
                                  )}
                                </div>
                            ...
                            {/* Nível 3: Sub-itens / Seções abaixo do Capítulo (Nível da atividade) */}
                            {expanded && (
        ...
        <section className="space-y-3 lg:hidden">
          ...
          {tomeGroups.map(tome => {
            ...
            return (
              <div key={tome.tomeCode} className="space-y-2">
                {/* Nível 1 Mobile: Cabeçalho do Tomo */}
                <div
                  onClick={() => toggleTome(tome.tomeCode)}
                  className="flex items-center justify-between gap-2 p-3 bg-muted/80 rounded-md border border-border/80 cursor-pointer select-none"
                >
                  <div className="flex items-center gap-2 min-w-0">
                    <span className="font-mono text-[11px] font-bold px-2 py-0.5 rounded bg-primary text-primary-foreground shrink-0">
                      {tome.tomeCode}
                    </span>
                    <span className="text-xs font-bold text-foreground truncate">{tome.tomeTitle}</span>
                  </div>
                ...
                {/* Nível 2 & 3 Mobile: Capítulos e Seções abaixo */}
                {isTomeOpen && (
                  <div className="space-y-2 pl-2">
                    {tome.chapters.map(chapter => {
                      ...
                      return (
                        <div key={chapter.sectionCode} className="technical-panel overflow-hidden">
                          <button
                            type="button"
                            onClick={() => toggleSection(chapter.sectionCode)}
                          ...
                          >
                            <div className="flex items-center gap-2 min-w-0">
                              ...
                              <span className="text-xs font-semibold truncate">{chapter.sectionTitle}</span>
                            </div>
                            <div className="flex items-center gap-2 shrink-0">
